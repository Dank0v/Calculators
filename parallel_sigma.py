...
def calculate_uncertainty(*args):
    try:
        ...
        dR_eq_dR1 = (R2**2) / (R1 + R2)**2
        dR_eq_dR2 = (R1**2) / (R1 + R2)**2
        
        # Calculate uncertainty of equivalent resistance
        sigma_R_eq = math.sqrt((dR_eq_dR1 * sigma1)**2 + (dR_eq_dR2 * sigma2)**2)
...
root.geometry("450x700")  # Start with a larger size
root.minsize(450, 700)    # Increased minimum size to ensure all elements are visible
...
# Title
title_label = ttk.Label(main_frame, text="Parallel Resistance Calculator", style="Header.TLabel")
